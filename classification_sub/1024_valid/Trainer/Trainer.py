...
def train(model,train_loader,optimizer,criterion,DEVICE,model_name='baseline'):
    ...
    if model_name == 'baseline_multi':
        ...
        for batch_idx,(image,sublabel) in tqdm(enumerate(train_loader)):
            ...
            loss_res = criterion(res, sublabel['residue'])
            loss_col = criterion(col, sublabel['color'])
            loss_tur = criterion(tur, sublabel['turbidity'])
            ...
            correct_res += prediction_res.eq(sublabel['residue'].view_as(prediction_res)).sum().item()# 아웃풋이 배치 사이즈 32개라서.
            
            prediction_col = col.max(1,keepdim=True)[1] # 가장 확률이 높은 class 1개를 가져온다.그리고 인덱스만
            correct_col += prediction_col.eq(sublabel['color'].view_as(prediction_col)).sum().item()# 아웃풋이 배치 사이즈 32개라서.

            prediction_tur = res.max(1,keepdim=True)[1] # 가장 확률이 높은 class 1개를 가져온다.그리고 인덱스만
            correct_tur += prediction_tur.eq(sublabel['turbidity'].view_as(prediction_tur)).sum().item()# 아웃풋이 배치 사이즈 32개라서.

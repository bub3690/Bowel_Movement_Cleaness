...
class ResLayer_multilabel_stage2_base_cnn_freeze(nn.Module):
    def __init__(self,sublabel_count,DEVICE,check_point):
        super(ResLayer_multilabel_stage2_base_cnn_freeze, self).__init__()
        #여기서 모델 체크포인트 읽어오기.
...
def model_initialize(sublabel_count,DEVICE,model_name='baseline',check_point=''):
    ...
    elif model_name == 'sub_2stage_cnn_freeze':
        model = ResLayer_multilabel_stage2_base_cnn_freeze(sublabel_count,DEVICE,check_point).to(DEVICE)
    else:

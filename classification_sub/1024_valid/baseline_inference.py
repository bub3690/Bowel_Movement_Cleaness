...
def main():
    ...
    if args.add_seg==False:
        X_images = glob('../../data/bmc_label_voc/JPEGImages/*.jpg') # test 폴더만 남기기
        X_train, X_valid = train_test_split(X_images, test_size=0.3,random_state=args.seed)
        X_valid, X_test = train_test_split(X_valid, test_size=0.3,random_state=args.seed) # split이 아니라 바로 inference 하도록 고치기 
    else:
    ...
    print("검증 셋 : ",len(Y_test_df),Counter(Y_test_df['label']))
    print("---")
    ...
    test_loader = load_dataloader(X_valid, Y_test_df, sublabel, BATCH_SIZE, args.model, args.augment, is_train = False, num_workers=args.num_workers)
